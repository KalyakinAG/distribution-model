...
Функция Полное(Значение = Истина) Экспорт
	ПолноеРаспределение = Значение;
	Возврат ЭтотОбъект;
КонецФункции
	...
	Если Поле = "*" Тогда
		...
		Если ТипИсточника = "Отношение" Тогда
			...
			Если ТипПоля = "ИзмеренияТаблицы" Тогда
				Если Модель.Таблица.Свойство("Измерения", Поля) Тогда
		...
		Если ТипПоля = "Измерения" И Модель.Свойство("Отношение") Тогда
			Если Модель.Отношение.Свойство("ИзмеренияБазы", Поля) Тогда
				Для Каждого _Поле Из Поля Цикл
					Поле(_Поле.Псевдоним);
				КонецЦикла;
			КонецЕсли;
...
Процедура РаспределитьПоПорядку()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРаспределения Цикл
		...
		Для Каждого СтрокаБазыРаспределения Из ТаблицаБазыРаспределения Цикл
			...
			СтрокаРезультата = СтруктураСтроки(РезультатРаспределения);
			ЗаполнитьЗначенияСвойств(СтрокаРезультата, СтрокаТаблицы);
			ЗаполнитьЗначенияСвойств(СтрокаРезультата, СтрокаБазыРаспределения);
			СтрокаРезультата[ПолеРаспределения] = СуммаРаспределения;
...
Процедура РаспределитьПропорционально()
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРаспределения Цикл
		...
		СуммаРаспределения = ?(ПолноеРаспределение, ОстатокРаспределения, Мин(ОстатокРаспределения, ТаблицаБазыРаспределения.Итог(ПолеРаспределения)));
...
Процедура Инициализировать()
	...
	Если Модель <> Неопределено Тогда
		...
		Если Модель.Свойство("Таблица", ОписаниеТаблицы) И ОписаниеТаблицы.ЭтоИсточникИзОбъекта Тогда
			ТаблицыДляУдаления.Добавить(ОписаниеТаблицы.Псевдоним);
		КонецЕсли;
		Если Модель.Свойство("База", ОписаниеТаблицы) И ОписаниеТаблицы.ЭтоИсточникИзОбъекта Тогда
			ТаблицыДляУдаления.Добавить(ОписаниеТаблицы.Псевдоним);
		КонецЕсли;
		Если Модель.Свойство("Отношение", ОписаниеТаблицы) И ОписаниеТаблицы.ЭтоИсточникИзОбъекта Тогда
			ТаблицыДляУдаления.Добавить(ОписаниеТаблицы.Псевдоним);
		КонецЕсли;
